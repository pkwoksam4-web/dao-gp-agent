...
SPECS=[
 {'symbol':'000564.SZ','target_date':'2021-12-31','expected_term':'10转22.035714股','source':'THS_BONUS_HISTORY','url':'https://basic.10jqka.com.cn/000564/bonus.html'},
...
def decode(raw, content_type):
    ...
    # Respect declared legacy charset first; tolerate a few bad/mixed bytes because
    # the canonical raw payload and SHA are retained separately.
    ...
    if 'utf-8' in ct or 'utf8' in ct:
        return raw.decode('utf-8-sig',errors='replace'),'utf-8-replace'
...
def main():
    ...
    for s in SPECS:
        ...
        html,enc=decode(raw,ct) if raw else ('',None)
        text=compact_text(html)
        date_pos=text.find(s['target_date']); term_pos=text.find(s['expected_term'])
        if date_pos>=0:
            lo=max(0,date_pos-1000); hi=min(len(text),date_pos+1800); context=text[lo:hi]
        else: context=''
        ...
        rows.append(rec)
        print(json.dumps({'symbol':s['symbol'],'status':rec['status'],'date':rec['target_date_found'],'term':rec['expected_term_found'],'near':rec['target_term_near'],'http':status,'decode':enc},ensure_ascii=False),flush=True)
