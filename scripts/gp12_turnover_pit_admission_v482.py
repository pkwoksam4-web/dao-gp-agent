...
def audit_replay(baseline_rows: Iterable[dict], replay_rows: Iterable[dict]) -> dict:
    ...
    for k in common:
        ...
        if fields:
            mismatches.append({"symbol": k[0], "date": k[1], "fields": fields,
                               "baseline": {f: base[k].get(f) for f in fields},
                               "replay": {f: replay[k].get(f) for f in fields}})
    missing = sorted(bk-rk); extra = sorted(rk-bk)
    status = "PASS_REPLAY_EXACT" if not missing and not extra and not mismatches else "REVIEW_REPLAY_EXACT"
    return {"status":status,"expected_n":len(base),"matched_n":len(common)-len(mismatches),
            "missing_n":len(missing),"extra_n":len(extra),"mismatch_n":len(mismatches),
            "missing": [list(x) for x in missing[:100]], "extra": [list(x) for x in extra[:100]],
...
def audit_prefix_invariance(full_rows: Iterable[dict], prefix_rows: Iterable[dict], cutoff: str) -> dict:
    ...
    return {**x, "status": "PASS_PREFIX_INVARIANCE" if x["status"] == "PASS_REPLAY_EXACT" else "REVIEW_PREFIX_INVARIANCE",
            "cutoff": cutoff}
...
def audit_adjustflag_invariance(rows_by_flag: dict[str, Iterable[dict]]) -> dict:
    ...
    details=[]
    symbols=set()
    for r in base: symbols.add(_key(r)[0])
    for flag in flags:
        if flag == "3": continue
        x=audit_replay(base, rows_by_flag[flag])
        total_mismatch += x["mismatch_n"]; total_missing += x["missing_n"]; total_extra += x["extra_n"]
        details.append({"flag":flag,**x})
    status="PASS_ADJUSTFLAG_INVARIANCE" if total_mismatch==total_missing==total_extra==0 else "REVIEW_ADJUSTFLAG_INVARIANCE"
    return {"status":status,"symbol_n":len(symbols),"flags":flags,"mismatch_n":total_mismatch,
            "missing_n":total_missing,"extra_n":total_extra,"details":details}


def build_turnover_pit_admission(structural: dict, replay: dict, prefix: dict, adjustflag: dict) -> dict:
    structural_ok=(structural.get("status")=="PASS_STRUCTURAL_PITST_ALIGNED_TURNOVER"
                   ...
                   and structural.get("turn_missing_n")==0 and structural.get("turn_negative_n")==0)
    if not structural_ok: raise ValueError("structural turnover coverage is not closed")
    replay_ok=(replay.get("status")=="PASS_REPLAY_EXACT" and replay.get("expected_n")==1011607
               and replay.get("missing_n")==0 and replay.get("extra_n")==0 and replay.get("mismatch_n")==0)
    if not replay_ok: raise ValueError("historical replay is not exact")
    prefix_ok=(prefix.get("status")=="PASS_PREFIX_MATRIX_INVARIANCE" and prefix.get("symbol_n")==847
               and prefix.get("probe_n")==847 and prefix.get("missing_n")==0
               and prefix.get("extra_n")==0 and prefix.get("mismatch_n")==0)
    if not prefix_ok: raise ValueError("historical prefix invariance is not closed")
    flag_ok=(adjustflag.get("status")=="PASS_ADJUSTFLAG_INVARIANCE" and adjustflag.get("symbol_n")==50
             and adjustflag.get("missing_n")==0 and adjustflag.get("extra_n")==0 and adjustflag.get("mismatch_n")==0)
    if not flag_ok: raise ValueError("adjustflag invariance is not closed")
    return {
        "artifact":"GP12_TURNOVER_RATIO_PIT_ADMISSION_V482","version":"V4.82",
        "status":"PASS_TURNOVER_RATIO_PIT_ADMISSION",
        "formal_window":["2020-06-01","2026-04-17"],
        "scope":{"symbol_n":847,"trade_row_n":1011607,"sample_adjustflag_symbol_n":50},
        "pit_contract":{
            "source_native_daily_field":"BaoStock query_history_k_data_plus.turn",
            "source_unit":"percent","production_transform":"turn / 100.0",
